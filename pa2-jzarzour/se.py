...
class CustomModel2(nn.Module):
    def __init__(self):
        ...
        with torch.no_grad():
            ...
            self.layer2.bias = nn.Parameter(torch.tensor([0.0, 0.0,2.0,1.0], dtype=torch.float64))
...
def main():
    parser = argparse.ArgumentParser(description="Program with multiple functions")
    parser.add_argument("command", nargs="?", default="main", choices=["main", "genPlot"], help="Function to execute")
    args = parser.parse_args()
    if args.command == "genPlot":
        genPlot()
    else:
        test1()
        test2()
